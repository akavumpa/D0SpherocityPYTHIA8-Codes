...
void sethistodown(TH2D *r1__1) {
  ...
  r1__1->GetXaxis()->SetTickLength(0.06);
...
TString partname[4] = {"pThatp", "pThatnp", "pThatp", "pThatnp"};
...
void pThat_plotting_pndnp() {
  ...
  TH2D *hframe1 = new TH2D(
      "", ";p_{T}^{D^{0}-lead} (GeV/#it{c});#LT#hatp_{T}#GT (GeV/#it{c})", 100,
      0.1, 24.0, 100, 1.0, 40.0);
  TH2D *hframe2 = new TH2D("", ";p_{T}^{D^{0}-lead} (GeV/#it{c});CR-off/on",
                           100, 0.1, 24.0, 100, 0.9, 1.09);
  sethisto(hframe1);
  ...
  // TH2D *h = new TH2D("h2", "", 100, 0, 24.0, 100, 1.0, 40.0);
  // sethisto(h);
  // h->GetYaxis()->SetTitle("#LT#hatp_{T}#GT (GeV/#it{c})");
  // h->GetXaxis()->SetTitle("p_{T}^{D^{0}-lead} (GeV/#it{c})");
  ...
  TLegend *pt =
      new TLegend(0.14033, 0.796627, 0.290094, 0.873512, NULL, "brNDC");
  setlegendstyle(pt);
  //    pt->SetFillColor(0);
  pt->SetTextSize(0.07);
