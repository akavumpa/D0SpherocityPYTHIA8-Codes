...
void fB_exp_comp_plot() {

  TFile *f1 = new TFile("fB_MB_CRon_MPIon.root"); 
   TFile *f2 = new TFile("fB_MB_CRon_MPIoff.root"); 
    TFile *f3 = new TFile("fB_MB_CRoff_MPIon.root"); 
  // PYTHIA
  ...
  Int_t color[4] = {kBlue+1, kGreen + 1, kRed+ 1, kGreen + 2};
  Int_t markerstyle1[4] = {20, 23, 22, 21};
  //  Int_t markerstyle2[4] = {24, 25,27};
  Double_t markersize[4] = {1.8, 1.8, 1.8, 1.8};

  c->Draw();
  h1a[0] = (TH1D *)f1->Get(partname1[0]);
   h1a[1] = (TH1D *)f2->Get(partname1[0]);
    h1a[2] = (TH1D *)f3->Get(partname1[0]);
  // h1a->Draw("pesame");
  for (int j = 0; j < 14; j++) {
    x2er[j] = {h1a[0]->GetBinWidth(j + 1) / 2};
    y2[j] = h1a[0]->GetBinContent(j + 1);
    y2er[j] = {h1a[0]->GetBinError(j + 1)};
  }
  ...
  gdatastat->SetMarkerStyle(markerstyle1[3]);
  gdatastat->SetMarkerSize(markersize[3]);
   for (int i = 0; i < 3; i++) {
  ...
  TLegend *pt = new TLegend(0.15508, 0.7727, 0.6631, 0.887, NULL, "brNDC");
  ...
  pt->Draw("same");
  TLegend *legend =
      new TLegend(0.6657, 0.167, 0.913, 0.398, NULL, "brNDC");
  // legend->SetHeader("Multiplicity, "+partID1[j],"C");
  setlegendstyle(legend);
  legend->AddEntry(gdatastat, "ALICE (INEL>0)", "lp");
  legend->AddEntry((TObject*)0, "  PYTHIA8", "");
  legend->AddEntry(h1a[0], "CR on MPI on ", "lp");
  
  legend->AddEntry(h1a[1], "CR on MPI off", "lp");
  // legend->AddEntry((TObject*)0, "PYTHIA8", "");

  legend->AddEntry(h1a[2], "CR off MPI on", "lp");
