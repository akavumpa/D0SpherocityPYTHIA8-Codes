...
void AvgD0NchVsNmpi_plotting() {
  ...
  TH2D *h2 = new TH2D("h2", "", 2, 0.5, 14.0, 400, 0.1, 50.0); // fig2
  sethisto(h2);
  h2->GetYaxis()->SetTitle("dN/dy/#LTdN/dy#GT ");
  h2->GetXaxis()->SetTitle("N_{mpi}/#LTN_{mpi}#GT");

  TGraphErrors *h[6];
  TString partname[6] = {"grMPIvsD0p",  "grMPIvsD0p", "grMPIvsD0np",
                         "grMPIvsD0np", "grMPIvsNch", "grMPIvsNch"};
  ...
  TLine *line = new TLine(0.5, 0.5, 14.0, 14.0);
  line->SetLineStyle(2);      // Dashed
  line->SetLineColor(kBlack); // Black line
  line->SetLineWidth(2);      // Thickne
  line->Draw("same");
  ...
  //    pt->SetBorderSize(0);
  pt->Draw("same");
  c->SaveAs("D0NchCRoffndonMPI.pdf");
}
